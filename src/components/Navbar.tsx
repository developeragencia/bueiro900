...
export default function Navbar() {
  ...
  return (
    <nav className={`sticky top-0 w-full z-50 transition-all duration-300 ${
      scrolled 
        ? "bg-gradient-to-r from-blue-900 to-indigo-800 shadow-md" 
      ...
      <div
        className={`fixed inset-0 bg-black/50 z-50 transition-opacity duration-300 ${
          mounted && mobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
        }`}
        onClick={() => setMobileMenuOpen(false)}
      >
        <div
          className={`absolute right-0 top-0 h-full w-64 bg-gradient-to-b from-blue-900 to-indigo-800 text-white shadow-xl transform transition-transform duration-300 ${
